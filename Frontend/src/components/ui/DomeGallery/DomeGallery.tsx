...
export default function DomeGallery({
  ...
  useEffect(() => {
    const AUTO_SPEED = 0.08; // degrees per frame (~4.8 deg/sec at 60fps)
    const tick = () => {
      // Pause auto-rotation while dragging, inertia is active, or an image is open
      if (!draggingRef.current && !inertiaRAF.current && !focusedElRef.current) {
        const nextY = wrapAngleSigned(rotationRef.current.y + AUTO_SPEED);
